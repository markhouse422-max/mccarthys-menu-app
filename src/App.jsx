...
const TAG = {
  V:     { color: "#66bb6a", bg: "rgba(102,187,106,0.22)" },
  VG:    { color: "#43a047", bg: "rgba(67,160,71,0.22)" },
  GF:    { color: "#e6a817", bg: "rgba(230,168,23,0.2)" },
  "GF*": { color: "#e6a817", bg: "rgba(230,168,23,0.15)" },
  DF:    { color: "#42a5f5", bg: "rgba(66,165,245,0.2)" },
  LF:    { color: "#42a5f5", bg: "rgba(66,165,245,0.15)" },
};
...
export default function App() {
    ...
    return(<div style={S.page}><Fonts/>
      ...
      <div ref={ref} style={S.content}>
        <div style={S.venueCard}>
          <div style={{fontSize:"16px",fontFamily:F.display,fontWeight:700,color:"#f0ead6",marginBottom:"5px"}}>{mMeal.venue}</div>
          <div style={{fontSize:"13px",color:"rgba(240,234,214,0.5)",lineHeight:1.6}}>
        ...
        {mMeal.sections&&mMeal.sections.map(function(sec,si){
          ...
          return(<div key={si} style={{marginBottom:"22px"}}>
            <div style={{display:"flex",alignItems:"center",gap:"10px",marginBottom:"10px"}}>
              <h3 style={{fontSize:"15px",fontFamily:F.body,fontWeight:600,color:"#daa520",margin:0,whiteSpace:"nowrap"}}>{sec.title+" – Select One"}</h3>
              ...
              return(<button key={ii} onClick={function(){pick(mDay.day,mMeal.type,sec.title,active?null:item.name);}} style={{...S.menuItem,background:active?"rgba(218,165,32,0.12)":"rgba(240,234,214,0.04)",border:active?"1px solid rgba(218,165,32,0.4)":"1px solid rgba(240,234,214,0.06)"}}>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
                  <div style={{flex:1}}>
                    ...
                    {item.desc&&<div style={{fontSize:"12px",color:"rgba(240,234,214,0.45)",marginTop:"4px",paddingLeft:item.num?"30px":0}}>{item.desc}</div>}
                  </div>
        ...
        {isComplete&&(<button onClick={function(){setView("day");}} style={{...S.goldBtn,width:"100%",marginTop:"20px",padding:"14px",fontSize:"14px",display:"flex",alignItems:"center",justifyContent:"center",gap:"8px"}}><span>✓</span> {"Selections Complete — Back to Day "+mDay.day}</button>)}
      </div>
    </div>);
  }
  ...
  return(<div style={S.page}><Fonts/>
    <div style={{padding:"16px 20px 0",maxWidth:"540px",margin:"0 auto"}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
        <div><div style={S.brandLabel}>McCarthy's Party</div><h1 style={{margin:0,fontSize:"21px",fontFamily:F.body,fontWeight:700,color:"#f0ead6"}}>Your Tour Menus</h1></div>
...
function Num({n,active}){return <span style={{display:"inline-flex",alignItems:"center",justifyContent:"center",width:"22px",height:"22px",borderRadius:"50%",background:active?"#daa520":"rgba(218,165,32,0.2)",color:active?"#1a2332":"#daa520",fontSize:"12px",fontWeight:700,flexShrink:0}}>{n}</span>}
function Radio({on}){return <div style={{width:"20px",height:"20px",borderRadius:"50%",border:"2px solid "+(on?"#daa520":"rgba(240,234,214,0.2)"),display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}>{on&&<div style={{width:"10px",height:"10px",borderRadius:"50%",background:"#daa520"}}/>}</div>}
function DietTag({t}){return <span style={{fontSize:"10px",padding:"3px 7px",borderRadius:"4px",background:TAG[t]?.bg||"rgba(136,136,136,0.2)",color:TAG[t]?.color||"#888",fontWeight:700,letterSpacing:"0.3px"}}>{t==="GF*"?"GF OPT":t}</span>}
